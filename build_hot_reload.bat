...
:: app and it also makes sure we don't have so many PDBs laying around.
if %APP_RUNNING% == false (
	del /q app_*.dll 2> nul
...
:: Build app dll, use pdbs\app_%PDB_NUMBER%.pdb as PDB name so each dll gets
...
:: the debugger is attached. So each time we compile `app.dll` we give the
:: PDB a unique PDB.
::
:: Note that we could not just rename the PDB after creation; the DLL contains a
:: reference to where the PDB is.
::
:: Also note that we always write app.dll to the same file. app_hot_reload.exe
:: monitors this file and does the hot reload when it changes.
echo Building app.dll
odin build app -strict-style -vet -debug -define:RAYLIB_SHARED=true -build-mode:dll -out:app.dll -pdb-name:pdbs\app_%PDB_NUMBER%.pdb > nul
IF %ERRORLEVEL% NEQ 0 exit /b 1

:: If app.exe already running: Then only compile app.dll and exit cleanly
if %APP_RUNNING% == true (
	echo App running, hot reloading... && exit /b 1
)

:: Build app.exe, which starts the program and loads app.dll och does the logic for hot reloading.
echo Building %EXE%
